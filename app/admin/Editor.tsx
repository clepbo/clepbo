...
function Field({
  label, value, onChange, area, ai, context, say, rows,
}: {
  label: string; value: string; onChange: (v: string) => void;
  area?: boolean; ai?: boolean; context?: string; say?: Shared["say"]; rows?: number;
}) {
  ...
  async function run(mode: string) {
    ...
    try {
      const res = await fetch("/api/ai", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ mode, text: value, context: context ?? label }),
      });
      const body = await res.json();
...
function ProjectForm({ doc, index, edit, say }: { doc: Content; index: number } & Shared) {
  ...
  return (
    <div className="card2__open">
      ...
      <Field label="Card line" value={p.line} ai say={say} context="the one-line summary on a portfolio project card"
        onChange={(v) => set((x) => { x.line = v; })} />
      <Field label="Story" value={p.story} area rows={5} ai say={say} context="the opening paragraph of a portfolio case study"
...
function CaseForm({ doc, index, edit, say }: { doc: Content; index: number } & Shared) {
  ...
  return (
    <>
      <Field label="The problem" value={c.problem} area rows={5} ai say={say}
        ...
        {c.process.map((s, i) => (
          <div className="card2" key={i} style={{ background: "var(--sur2)" }}>
            ...
            <div style={{ padding: "0 .9rem .9rem" }}>
              <Field label="What you did" value={s.body} area ai say={say}
      ...
      <div className="fld">
        ...
        {c.decisions.map((s, i) => (
          <div className="card2" key={i} style={{ background: "var(--sur2)" }}>
            ...
            <div style={{ padding: "0 .9rem .9rem" }}>
              <Field label="Why it beat the alternative" value={s.body} area ai say={say}
      ...
      <Field label="Outcome" value={c.outcome} area ai say={say}
